...
const ResetPasswordPage: React.FC = () => {
  ...
  // Slideshow images - same as login/signup pages
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 overflow-hidden">
      ...
      {isFormVisible && (
        <div className="w-full max-w-md fixed left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 group mx-4">
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 p-6 sm:p-8 relative">
            ...
            <button
              ...
              className="absolute top-3 right-3 p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-100 touch-manipulation"
              title="Hide form"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            <div className="mb-6 text-center">
              ...
              <div className="flex items-center justify-center space-x-3 mb-4">
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs sm:text-sm text-gray-500 font-small">Reset Your Password</p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-black text-white py-3 rounded-lg font-semibold hover:bg-black/85 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 shadow-sm hover:shadow-md touch-manipulation"
              >
                {loading ? (
                  <LoadingSpinner size="sm" />
                ) : (
      ...
      {!isFormVisible && (
        <button
          ...
          title="Show reset form"
        >
          <span className="text-white font-thin text-xs sm:text-sm lg:text-md">Reset Password</span>
        </button>
      )}
    </div>
  );
